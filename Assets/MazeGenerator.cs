...
public class MazeGenerator : MonoBehaviour {
    ...
    private void Awake()
    {
        mazeSeed = stageSelect.stageIndex;
    ...
    private void GenerateMaze(int rows, int columns)
    {
        ...
        Vector2 spawnPosition = new Vector2(Random.Range(0,9), Random.Range(0,12));
        SpawnGameObjectInMaze(redDotPrefab, spawnPosition);
        
         spawnPosition = new Vector2(0, 12);
        SpawnGameObjectInMaze(bugPrefab, spawnPosition);
    ...
    public void SpawnGameObjectInMaze(GameObject prefab, Vector2 position)
    {
        // Ensure the position is within the maze bounds
        if (position.x >= 0 && position.x < mazeColumns && position.y >= 0 && position.y < mazeRows)
        {
            Vector3 spawnPosition = new Vector3(-(cellSize * (mazeColumns / 2)) + (cellSize / 2) + position.x * cellSize,
                -(cellSize * (mazeRows / 2)) + (cellSize / 2) + position.y * cellSize,
                0);

            Instantiate(prefab, spawnPosition, Quaternion.identity);
            Debug.Log("spawned prefab: "+prefab);
        }
        else
        {
            Debug.LogWarning("Spawn position is outside the maze bounds.");
        }
    }
